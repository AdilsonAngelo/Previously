defmodule PreviouslyWeb.API.SessionControllerTest do
  use PreviouslyWeb.ConnCase

  alias Previously.{Repo, Users.User}
  ...
  describe "create/2" do
    ...
    test "with valid params", %{conn: conn} do
      conn = post(conn, Routes.api_session_path(conn, :create, @valid_params))
    ...
    test "with invalid params", %{conn: conn} do
      conn = post(conn, Routes.api_session_path(conn, :create, @invalid_params))
  ...
  describe "renew/2" do
    setup %{conn: conn} do
      authed_conn = post(conn, Routes.api_session_path(conn, :create, @valid_params))
      :timer.sleep(100)
